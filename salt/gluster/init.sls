...
{% set volumes_distributed_list = salt['pillar.get']('gluster:volumes_distributed', ['recover']) %}
...
/mnt/glusterfs/{{ volume }}:
  mount.mounted:
    ...
    - device: {{ glusterfs_host_list[0] }},{{ glusterfs_host_list[1] }},{{ glusterfs_host_list[2] }},{{ glusterfs_host_list[3] }},{{ glusterfs_host_list[4] }}:/{{ volume }}
    - fstype: glusterfs
